...
**Model container selection is environment-dependent** (in both `GiftListsApp` and `GiftListsWatchApp`): the simulator (and macOS DEBUG) loads `previewContainer` (in-memory, seeded with sample data from `Preview Content/PreviewContainer.swift`), while real devices use a persistent CloudKit-backed `.modelContainer(for:)`. When changing the model schema, update `PreviewContainer.swift` too or previews/simulator runs will break.
